...
impl S2nTlsHostConfig {
    pub(crate) fn build_tls_config(&self) -> anyhow::Result<s2n_tls::config::Config> {
        ...
        // TODO add cert and key pairs via s2n_config_add_cert_chain_and_key_to_store
        if let Some(pair) = self.cert_pairs.first() {
            builder
                .load_pem(pair.cert_chain(), pair.private_key())
                .map_err(|e| anyhow!("failed to load cert and key: {e}"))?;
        }
        ...
        if self.use_session_ticket {
            builder
                ...
                .map_err(|e| anyhow!("failed to enable session tickets: {e}"))?;
        }
